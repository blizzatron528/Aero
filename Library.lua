...
local Aero = {
	...
	InfoDisplay   = {FPS = nil, Ping = nil, Username = nil, Expiry = nil, Avatar = nil},
...
function Aero:Initialize()
	...
	Aero.InfoDisplay.Avatar = ProfileImage
	...
	Aero.InfoDisplay.Username = Username
	...
	Aero.InfoDisplay.Expiry = Expiry
	...
	function AeroWindow:Tab(Title, Image)
		...
		function AeroTab:Section(Side, Title, Description)
			...
			function AeroSection:SetTop(Value)
				...
				if #Instances < 1 then
					for i, v in Section:GetDescendants() do
						if HasProperty(v, "ZIndex") and v.ZIndex < 1000 then
			...
			function AeroSection:Toggle(Title, Settings)
				...
				AeroToggle:Set(AeroToggle.Value)
			...
			function AeroSection:Colorpicker(Title, Settings)
				...
				do
					...
					AeroColorpicker:UpdateColor(Color.R, Color.G, Color.B, AeroColorpicker.Transparency)
				end
...
local AeroLib = Aero:Initialize()

-- // Info Updating \\ --

if not isfolder("Aero\\Data") then
	makefolder("Aero\\Data")
end
...
LPH_NO_VIRTUALIZE(function()
	task.spawn(function()
		...
		local Time = LRM_SecondsLeft or 1792403813
		
		local function GetTimeLeft()
			local CurrentTime = os.time()
			local Diff = Time - CurrentTime
		...
		local ID = LPH_OBFUSCATED and tostring(LRM_LinkedDiscordID) or "745514175190663181"
		...
		local Success, Response = pcall(function()
			return request({
				...
				Method = "GET",
				Headers = {
					["Authorization"] = "Bot MTQyOTMxOTE2MDQwNzc4NTQ4Mg.GUBwBD.vdluQUpKZ-zlbRYA2W-ThnSGKiLNhQfPUYFoFE", -- // Empty bot that do nun :broke_heart:
				}
			})
		end)
		...
		if Success then
			...
			Aero.InfoDisplay.Username.Text = Data.username
			
			if Data.avatar then
				local Path = "Aero\\Data\\Avatar.png"
				local AvatarUrl = "https://cdn.discordapp.com/avatars/" .. Data.id .. "/" .. Data.avatar .. ".png?size=512"
				local Req  = request({Url = AvatarUrl}).Body
				writefile(Path, Req)
				Aero.InfoDisplay.Avatar.Image = getcustomasset(Path)
			end
		end
		
		local FPSCounter  = Aero.InfoDisplay.FPS
		local PingCounter = Aero.InfoDisplay.Ping
		local Expiry	  = Aero.InfoDisplay.Expiry
		...
		game:GetService("RunService").Heartbeat:Connect(function()
			...
			if Delta >= 1 then
				...
				PingCounter.Text = "Ping : "..tostring(math.round(Stats.Ping:GetValue()))
				Expiry.Text		 = GetTimeLeft()
